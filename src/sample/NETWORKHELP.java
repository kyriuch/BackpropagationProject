...
public class NETWORKHELP {
    ...
    final ArrayList<NEURONHELP> inputLayer = new ArrayList<NEURONHELP>();
    final ArrayList<NEURONHELP> hiddenLayer = new ArrayList<NEURONHELP>();
    final ArrayList<NEURONHELP> outputLayer = new ArrayList<NEURONHELP>();
    final NEURONHELP bias = new NEURONHELP();
    final int[] layers;
    ...
    public static void main(String[] args) {
        NETWORKHELP nn = new NETWORKHELP(2, 4, 1);
        int maxRuns = 50000;
        double minErrorCondition = 0.001;
        nn.run(maxRuns, minErrorCondition);
    }

    public NETWORKHELP(int input, int hidden, int output) {
        ...
        for (int i = 0; i < layers.length; i++) {
            if (i == 0) { // input layer
                for (int j = 0; j < layers[i]; j++) {
                    NEURONHELP neuron = new NEURONHELP();
                ...
                System.out.println("!Error NETWORKHELP init");
            }
        }

        // initialize random weights
        for (NEURONHELP neuron : hiddenLayer) {
            ArrayList<CONNECTIONHELP> connections = neuron.getAllInConnections();
            for (CONNECTIONHELP conn : connections) {
                double newWeight = getRandom();
                conn.setWeight(newWeight);
            }
        }
        for (NEURONHELP neuron : outputLayer) {
            ArrayList<CONNECTIONHELP> connections = neuron.getAllInConnections();
            for (CONNECTIONHELP conn : connections) {
                double newWeight = getRandom();
                conn.setWeight(newWeight);
        ...
        // reset id counters
        NEURONHELP.counter = 0;
        CONNECTIONHELP.counter = 0;
    ...
    public void activate() {
        for (NEURONHELP n : hiddenLayer)
            n.calculateOutput();
        for (NEURONHELP n : outputLayer)
            n.calculateOutput();
    }
    ...
    public void applyBackpropagation(double expectedOutput[]) {
        ...
        int i = 0;
        for (NEURONHELP n : outputLayer) {
            ArrayList<CONNECTIONHELP> connections = n.getAllInConnections();
            for (CONNECTIONHELP con : connections) {
        ...
        for (NEURONHELP n : hiddenLayer) {
            ...
            for (CONNECTIONHELP con : connections) {
                ...
                int j = 0;
                for (NEURONHELP out_neu : outputLayer) {
